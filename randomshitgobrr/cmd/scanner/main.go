...
//	Stage 2  DNS check  → A query per IP (fast, many workers)
...
// Usage:
//
//	hybrid-scanner -list FILE -sample N -a DOMAIN \
...
func main() {
	...
	var aDomainStr, tunnelDomainStr string
	var pubkeyFile, pubkeyHex string
	var dnsTimeout, hsTimeout time.Duration
	...
	flag.IntVar(&dnsWorkers, "dns-workers", 200, "concurrent A-query workers (stage 2)")
	flag.IntVar(&hsWorkers, "hs-workers", 50, "concurrent Noise handshake workers (stage 3)")
	flag.DurationVar(&dnsTimeout, "dns-timeout", 3*time.Second, "timeout for A query check")
	flag.DurationVar(&hsTimeout, "hs-timeout", 10*time.Second, "timeout for Noise handshake")
	...
	flag.Usage = func() {
		fmt.Fprintf(os.Stderr,
			"Usage: %s -list FILE -sample N -a DOMAIN -domain DOMAIN -pubkey-file FILE [flags]\n\n",
			os.Args[0])
		flag.PrintDefaults()
	}
	flag.Parse()

	if listFile == "" || aDomainStr == "" || tunnelDomainStr == "" {
		flag.Usage()
		os.Exit(1)
	}
	...
	// Load IPs before starting the TUI so warnings print cleanly.
	...
	go runScanner(ips, dnsPort, aDomainStr, tunnelDomain, pubkey, cfg,
		dnsWorkers, hsWorkers, dnsTimeout, hsTimeout, out, stats)
